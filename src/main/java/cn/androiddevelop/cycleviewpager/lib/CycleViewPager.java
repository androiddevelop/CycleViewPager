...
import cn.androiddevelop.cycleviewpager.base.CycleViewPagerIdleListener;
import cn.androiddevelop.cycleviewpager.base.MyHandler;
import cn.androiddevelop.cycleviewpager.example.util.LogUtil;
	...
	final Runnable runnable = new Runnable() {
		...
		public void run() {
			if (getActivity() != null && !getActivity().isFinishing()
				...
				LogUtil.log("time", "" + (now - releaseTime));
				// 检测上一次滑动时间与本次之间是否有触击(手滑动)操作，有的话等待下次轮播
				if (now - releaseTime > time - 500) {
					LogUtil.log("notice", "start to wheel");
					handler.sendEmptyMessage(WHEEL);
				} else {
					LogUtil.log("notice", "wait for next time to wheel");
	...
	private void setIndicator(int selectedPosition) {
		...
		if (indicators.length > selectedPosition)
			indicators[selectedPosition]
					.setBackgroundResource(R.drawable.cycleviewpager_indicator_circle_black);
	}
